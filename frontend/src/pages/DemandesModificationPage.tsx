...
export function DemandesModificationPage() {
  ...
  const [form, setForm] = useState<FormState>({ champModifie: 'email', nouvelleValeur: '' });
  ...
  const validerMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.post(`/demandes-modification/${id}/valider`);
    },
    ...
    <div className="space-y-6">
        ...
        actions={
          perms.canSubmitDemande ? (
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger>
                <Button>
                  <Plus className="mr-2 h-4 w-4" /> Proposer une modification
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Nouvelle demande de modification</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleCreateSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    <Label>Champ a modifier</Label>
                    <Select
                      value={form.champModifie}
                      onValueChange={(v) =>
                        setForm({ ...form, champModifie: v ?? 'email' })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
      ...
      {!query.data || query.data.length === 0 ? (
        <EmptyState
          ...
          description="Aucune demande de modification enregistree."
        />
      ) : (
        <Card>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Ancienne</TableHead>
                <TableHead>Nouvelle</TableHead>
                    ...
                    {perms.canValidateDemandes && d.statut === 'EN_ATTENTE' && (
                      <>
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => validerMutation.mutate(d.id)}
                          title="Valider"
                        >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                rejetOpen && rejeterMutation.mutate({ id: rejetOpen, motif })
              }
            >
